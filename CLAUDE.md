...
Summary messages carry an inline keyboard: **สรุปตอนนี้ / หยุดเข้าไม้ใหม่ / กลับมาเข้าไม้**.
`parse_updates()` is pure and **drops any update whose chat id is not the configured one** —
a Telegram bot answers whoever writes to it, so without that filter anyone who finds the
bot could halt the trading. There is deliberately no "close everything" button: pausing
entries gets the same outcome without a pocket tap closing live positions, and open trades
keep being managed while paused. `state["entries_paused"]` is persisted, because a pause
that a restart forgets is an instruction that was not followed. `take_commands()` polls
`getUpdates` once per cycle with an in-memory offset and swallows every failure.
